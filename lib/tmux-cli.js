...
function createTmuxCli(opts) {
  ...
  async function verbNewSurface(ctx, a) {
    ...
    const ws = target(a.flags.workspace, 'workspace');
    const pane = a.flags.pane ? target(a.flags.pane, 'pane') : null;
  ...
  async function verbNewWorkspace(ctx, a) {
    ...
    const r = pick
      ...
      : await must(ctx, ['new-session', '-d', '-s', session, '-P', '-F', fmt, ...dir], { effect: true });
  ...
  async function verbCloseSurface(ctx, a) {
    await ensure();
    const sf = target(a.flags.surface, 'surface');
    const ws = a.flags.workspace ? target(a.flags.workspace, 'workspace') : null;
    await preCheck(ctx, sf, ws);
    const g = await must(ctx, ['display', '-p', '-t', sf.tmux, `#{window_panes}${SEP}#{window_id}`]);
    const [count, wid] = g.stdout.trim().split(SEP);
    if (ws && wid !== ws.tmux) fail(`not_found: ${sf.id} is not in ${ws.id}`);
    if (Number(count) <= 1) fail('invalid_state: Cannot close the last surface');
    await must(ctx, ['kill-pane', '-t', sf.tmux], { effect: true });
    return 'OK';
  }
  ...
  async function verbWorkspaceAction(ctx, a) {
    ...
    if (action === 'rename') {
      await preCheck(ctx, ws);
      await must(ctx, ['rename-window', '-t', ws.tmux, '--', String(a.flags.title || '')], { effect: true });
      return 'OK';
    }
  ...
  async function verbFocusPane(ctx, a) {
    await ensure();
    const pane = target(a.flags.pane, 'pane');
    const ws = a.flags.workspace ? target(a.flags.workspace, 'workspace') : null;
  ...
  async function verbPaneResize(ctx, a) {
    await ensure();
    const p = parseJson(a.pos[2]);
    const me = target(p.pane_id || p.pane, 'pane');
