...
// pane TMUX_PANE=%N, and the bridge's ensure() puts CMUX_TMUX_EPOCH in the server's global
// environment before any shell starts. Either missing or malformed -> '' (radar falls back to cwd).
function surfaceFromEnv(env) {
  const e = env || {};
  const pane = String(e.TMUX_PANE || '');
  const epoch = String(e.CMUX_TMUX_EPOCH || '');
  if (!/^%\d+$/.test(pane) || !/^\d+$/.test(epoch)) return '';
